...
export default function Wanttoread({ navigation }) {
  ...
  const saveItem = (key, option) => {
    if (option == 1) {
        push(
            ref(database, `books/Reading`), books[key]
          )
    } else if (option == 2){
        push(
        ...
        renderItem={({ item, index }) =>
          ...
          <View style={{ justifyContent: 'space-evenly', flexDirection: 'row', marginTop: 10 }}>
            ...
            <Button onPress={() => {
              ...
              title='MOVE'
            />
            <Button onPress={() => navigation.navigate('BookFromShelf', {book: books[index], shelf: 'Want to read', index: keys[index]})} radius={'md'} type='outline' raised title='SHOW BOOK' />
          </View>
          </>}
      />
      <Dialog
